...
%                     %Aversive
%                     tmp = spiketrains_dHPC.pyr';
%                     tmp1 = spiketrains_vHPC.pyr';
%                     R.aversive = zeros(size(tmp1,2),1);
%                     for i =  1:size(tmp1,2)
%                         R.aversive(i) = tmp(:,i)' * S2 * tmp1(:,i);
%                     end
%                     R.aversive = smoothdata(R.aversive,'gaussian',1,'SamplePoints',bins);
%                     clear i
%                     %Reward
%                     R.reward = zeros(size(tmp1,2),1);
%                     for i =  1:size(tmp1,2)
%                         R.reward(i) = tmp(:,i)' * S4 * tmp1(:,i);
%                     end
%                     R.reward = smoothdata(R.reward,'gaussian',1,'SamplePoints',bins);
%                     clear Sx Sy Sz ev rev S1 S2 S3 S4 S5 i tmp tmp1
%                     
%                     R.split.baseline.reward = [];
%                     R.split.baseline.aversive = [];
%                     for i = 1:size(ripple_bursts.baseline,1)
%                         ii = InIntervals(bins,[ripple_bursts.baseline(i,2)-2 ripple_bursts.baseline(i,2)+2]);
%                         R.split.baseline.reward = [R.split.baseline.reward ; R.reward(ii)'];
%                         R.split.baseline.aversive = [R.split.baseline.aversive ; R.aversive(ii)'];
%                     end
%                     
%                     R.split.reward.reward = [];
%                     R.split.reward.aversive = [];
%                     for i = 1:size(ripple_bursts.baseline,1)
%                         ii = InIntervals(bins,[ripple_bursts.baseline(i,2)-2 ripple_bursts.baseline(i,2)+2]);
%                         R.split.reward.reward = [R.split.reward.reward ; R.reward(ii)'];
%                         R.split.reward.aversive = [R.split.reward.aversive ; R.aversive(ii)'];
%                     end  
%                     
%                     R.split.aversive.reward = [];
%                     R.split.aversive.aversive = [];
%                     for i = 1:size(ripple_bursts.baseline,1)
%                         ii = InIntervals(bins,[ripple_bursts.baseline(i,2)-2 ripple_bursts.baseline(i,2)+2]);
%                         R.split.aversive.reward = [R.split.reward.aversive ; R.reward(ii)'];
%                         R.split.aversive.aversive = [R.split.reward.aversive ; R.aversive(ii)'];
%                     end                      
                end
            end
